      ...
      local choose = function(player, choice)
        ...
        if vname then
          ...
          if vehicle then
			...
			vRPclient.notify(source,{"Asteapta 15 secunde."})
			vRPclient.notify(source,{"Alvin e de vina."})
...
local function ch_call(player,choice)
    vRP.prompt(player,"Player ID(scrie 'inchide' pentru a inchide apelul):","",function(player,target)
        if not (target == "inchide") then
            popei = vRP.getUserSource(tonumber(target))
            --print("Server id : ," .. popei .. "vRP Id : " .. target)
        else
            popei = "inchide"
            --print('Server id : ,'..popei..'vRP Id : ' .. target)
        end
        local player = source
        TriggerEvent('dvcall',player,popei)
    end)
end
...
vRP.registerMenuBuilder("main", function(add, data)
  ...
  if user_id ~= nil then
	...
	choices["Apeleaza Jucator Voce"] = {ch_call}
